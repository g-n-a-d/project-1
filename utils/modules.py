    ...
    def __init__(self, inputDim, outputShape=(224, 224), numLayer=3):
        ...
        for i in range(1, numLayer):
            layerList.append(torch.nn.Sequential(
                torch.nn.Conv1d(inputDim//(2**i), inputDim//(2**(i + 1)), kernel_size=7, padding=3),
                torch.nn.ReLU(),
                torch.nn.AvgPool1d(kernel_size=2)
            ))
            # layerList.append(torch.nn.Conv1d(inputDim//(2**i), inputDim//(2**(i + 1)), kernel_size=7, padding=3))
        self.down = torch.nn.Sequential(*layerList)
        self.sampler = torch.nn.Upsample(size=outputShape)
